...
import { getMentorUpcomingSessions, getMentorSessionHistory, markSessionAsCompleted } from "@/services/mentor/mentor";
import {
    CalendarIcon,
    ClockIcon,
    VideoCameraIcon,
...
const BookingsPage = () => {
    ...
    const fetchSessions = async () => {
        ...
        try {
            ...
            const upcomingData = await getMentorUpcomingSessions(userId);
            if (!upcomingData.error) {
                setUpcomingSessions(upcomingData || []);
            }

            const historyData = await getMentorSessionHistory(userId);
    ...
    const handleMarkComplete = async (bookingId) => {
        ...
        try {
            const userId = localStorage.getItem("userId");
            const res = await markSessionAsCompleted(bookingId, userId);
            if (res.error) {
    ...
    const formatTime = (timeString) => {
        ...
        // If already in 12h format or includes AM/PM, return as is (simple check)
    ...
    const SessionCard = ({ session, isHistory = false }) => (
        <div className="group bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col h-full">
            <div className="p-6 flex-grow">
                ...
                <div className="mb-6">
                    <h3 className="text-xl font-bold text-gray-900 leading-tight mb-2 break-words">{session.user_name || "Unknown User"}</h3>
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#fcfdfe]">
            <Header />
            <Loader isLoading={loading} message="Fetching session details..." />
                ...
                <div className="max-w-[1600px] mx-auto">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                        <div>
                            ...
                            <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight">Mentorship Sessions</h1>
                                ...
                                sessionHistory.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[40px] border border-dashed border-gray-200">
                                        ...
                                        <p className="text-gray-400 text-sm max-w-xs text-center">
                                            Your completed and past sessions will be archived here.
                                        </p>
                                    </div>
                                ) : (
                                    <div className="grid gap-8 md:grid-cols-2 xl:grid-cols-3">
