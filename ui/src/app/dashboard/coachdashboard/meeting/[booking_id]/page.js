...
export default function CoachMeetingPage() {
  ...
  const handleLeave = () => {
    router.push("/dashboard/coachdashboard/sessions");
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#fcfdfe]">
      ...
      <main className="flex-grow w-full">
        ...
        {/* Info message for mentors - Above Jitsi container */}
        {meetingData.userInfo?.role === 'mentor' && (
          <div className="w-full px-4 md:px-8 py-2">
            <div className="max-w-7xl mx-auto bg-green-50 border border-green-200 rounded-lg p-3 text-sm text-green-800">
              <p className="font-semibold">👋 Welcome Coach!</p>
              <p>Click "Join" to start the meeting. You'll become the moderator automatically.</p>
            </div>
          </div>
        )}
        ...
        <div className="w-full h-[calc(100vh-140px)] p-2 md:p-4">
          <div className="max-w-7xl mx-auto h-full bg-white rounded-2xl shadow-lg overflow-auto relative">
            <JitsiMeet
              ...
              userRole={meetingData.userInfo?.role || 'mentor'}
              onLeave={handleLeave}
