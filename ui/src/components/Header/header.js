...
export default function Header() {
    ...
    return (
        <>
            ...
            <header className="bg-gradient-to-r from-[#0A3551] via-[#0f4a70] to-[#0A3551] sticky top-0 shadow-md w-full z-50 border-b border-[#1a5a7a]">
                <div className="container-professional !py-0 px-2 lg:px-4">
                    <div className="flex flex-row justify-between items-center py-1">

                        {/* ✅ Logo on Left */}
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 md:w-11 md:h-11 rounded-full overflow-hidden border-2 border-white shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl ring-1 ring-white/20">
                                <Image
                        ...
                        <div className="flex items-center">
                            <Menu as="div" className="relative inline-block text-left">
                                <div>
                                    <MenuButton className="inline-flex items-center justify-center w-10 h-10 md:w-11 md:h-11 rounded-lg bg-white/95 hover:bg-white border hover:border-white shadow-md hover:shadow-lg transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-1 focus:ring-offset-[#0A3551] p-2">
                                        <User className="w-5 h-5 md:w-6 md:h-6 text-[#0A3551]" />
                                    </MenuButton>
                                </div>
                                <Transition
                                    as={Fragment}
                                                ...
                                                {({ active }) => (
                                                    <button
                                                        ...
                                                        className={`${active ? 'bg-red-50 text-red-700' : 'text-gray-700'
                                                            } group flex w-full items-center rounded-xl px-4 py-3.5 text-sm font-semibold transition-all duration-200 ${active ? 'shadow-md' : ''
                                                            }`}
                                                    >
                                                        <LogOut
                                                            className={`mr-3 h-5 w-5 ${active ? 'text-red-600' : 'text-red-500'
